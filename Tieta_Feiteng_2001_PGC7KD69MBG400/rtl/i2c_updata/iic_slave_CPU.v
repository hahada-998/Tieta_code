// 2025-06-12    wanglin         2.0  
// 1,增加判断当IIC master发送READ_BYTE命令时，状态机跳转到读数据，解决iic_scan挂死问题  
`timescale 1ns / 1ps

module iic_slave_CPU(
input wire reset_n,
		...
		case(state)
			...
			STATE_ACK1:
				begin
					if(iic_scl_negedge == 1'b1)
						begin
							if(command[7:0] == {DEVICE_ID,1'b0})//add wanglin
								begin
							...
							else if(command[7:0] == {DEVICE_ID,1'b1})//add wanglin
								state <= STATE_RDATA;
							else
								state <= STATE_IDLE;
						end
